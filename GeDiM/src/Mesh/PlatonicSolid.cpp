...
GeometryUtilities::Polyhedron PlatonicSolid::dual_polyhedron(const GeometryUtilities::Polyhedron& polyhedron) const
{
    ...
    Gedim::MeshUtilities::ComputeMesh2DCell1DsResult result = meshUtilities.ComputeMesh2DCell1Ds(dual.Vertices,
                                                                                                 faces);
                                                                             ...
                                                                             const unsigned int &frequency) const
{
    ...
    unsigned int id_vertices = mesh.Cell0DAppend((frequency - 1) * numCell1Ds + (0.5 * (frequency - 1) * (frequency - 2)) * numCell2Ds);
    ...
    for(unsigned int f = 0; f < numCell2Ds; f++)
    {
        ...
        mesh.Cell1DUpdatedCell1Ds(first_edge,
                                  first_tmp);
        ...
        mesh.Cell1DUpdatedCell1Ds(second_edge,
                                  second_tmp);
        ...
        unsigned int id_edges = mesh.Cell1DAppend(1.5 * frequency * (frequency + 1) - 2);
        {
            ...
            mesh.Cell1DInsertExtremes(id_edges,
                                      v2_first, v2_second);
            ...
            face << v2_first, second_vertex, v2_second,
                first_edge_subcells[0], second_edge_subcells[0], id_edges-1;
        ...
        for(unsigned int level = 1; level < frequency - 1; level++)
        {
            ...
            for(unsigned p = 1; p < level + 1; p++)
            {
                ...
                if(p == 1)
                {
                    ...
                    Eigen::MatrixXi face(2, 3);
                    face << v2_first, v1_first, id_vertices,
                        first_edge_subcells[level], id_edges-1, id_edges-2;
                    subCells.push_back(face);
                }
                else
                {
                    ...
                    Eigen::MatrixXi face(2, 3);
                    face << jj, id_vertices, id_vertices - 1,
                        id_edges - 1, id_edges - 2, id_edges - 3;
                    subCells.push_back(face);
                }
                ...
                if(p == level)
                {
                    ...
                    mesh.Cell1DInsertExtremes(id_edges,
                                              id_vertices, v1_second);
                    mesh.Cell1DSetMarker(id_edges, 0);
                    mesh.Cell1DSetState(id_edges, true);
                    id_edges++;

                    Eigen::MatrixXi face1(2, 3);
                    face1 << id_vertices, v1_second, v2_second,
                        id_edges-1, second_edge_subcells[level], id_edges-2;
                    ...
                    face2 << v1_second, id_vertices, mesh.Cell1DOrigin(id_horizontal_edge),
                        id_edges-1, id_edges-3, id_horizontal_edge;
                ...
                else
                {
                    ...
                    Eigen::MatrixXi face(2, 3);
                    face << jj, id_vertices, mesh.Cell1DOrigin(id_previous_horizontal_edge + 3 * (p - 1)),
        ...
        // last level
        {
            ...
            mesh.Cell1DUpdatedCell1Ds(third_edge,
                                      third_tmp);
            ...
            for(unsigned p = 1; p < level + 1; p++)
            {
                ...
                if(p == 1)
                {
                    ...
                    Eigen::MatrixXi face(2, 3);
                    face << v2_first, v1_first, last_id_vertices,
                        first_edge_subcells[level], id_edges-1, third_edge_subcells[p - 1];
                    subCells.push_back(face);
                }
                else
                {
                    ...
                    Eigen::MatrixXi face(2, 3);
                    face << jj, last_id_vertices, origin_id,
                        id_edges - 1, third_edge_subcells[p - 1], id_edges - 2;
                    subCells.push_back(face);
                }
                ...
                if(p == level)
                {
                    ...
                    mesh.Cell1DInsertExtremes(id_edges,
                                              last_id_vertices, v1_second);
                    mesh.Cell1DSetMarker(id_edges, 0);
                    mesh.Cell1DSetState(id_edges, true);
                    id_edges++;

                    Eigen::MatrixXi face1(2, 3);
                    face1 << last_id_vertices, v1_second, v2_second,
                        id_edges-1, second_edge_subcells[level], third_edge_subcells[p];
                    subCells.push_back(face1);
                    ...
                    Eigen::MatrixXi face2(2, 3);
                    face2 << v1_second, last_id_vertices, mesh.Cell1DOrigin(id_horizontal_edge),
                        id_edges-1, id_edges-2, id_horizontal_edge;
                    subCells.push_back(face2);
                }
                else
                {
    ...
    const auto extract_mesh_data = meshUtilities.ExtractMesh2D(filter_active_mesh.Cell0Ds,
                                                               filter_active_mesh.Cell1Ds,
                                                               filter_active_mesh.Cell2Ds,
                                                               mesh,
                                                               filter_mesh);
